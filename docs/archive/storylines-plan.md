...
**Status:** closed 2026-05-12. Server cycle (W1-W12 + W10 acceptance gate), webapp cycle, and multi-entity follow-up all shipped 2026-05-12 (verified in the browser). Current state of the feature lives in [`../storylines.md`](../storylines.md) (server) and [`../../../webapp/docs/storylines.md`](../../../webapp/docs/storylines.md). Follow-up cycle (MCP discoverability + append mode) is archived at [`storylines-2026-05-mcp-and-append.md`](./storylines-2026-05-mcp-and-append.md). **Last updated:** 2026-05-12. **Supersedes:** none.
...
1. ~~**Citation granularity.**~~ **Closed 2026-05-12** (W10 retread passed). The narrator now builds one `source="text"` document per entry; citations carry the `char_location` shape and `cited_text` is a sentence-level excerpt. Visual confirmation against prod: Atlas narrative panel renders inline citation quotes around 70-125 chars (e.g. `[1]` = "I'm almost 40, I'm a husband, a dad to Atlas (7) and Vienna (10) & a freelance data engineer..."; `[3]` = "The kids are fighting - but, now atlas is in his room feeling shame.") — sentence-length, not the 1000+ char wrapped blobs `source="content"` would have produced. Narrative voice stayed restrained; kill criterion #1 did not fire. Webapp `<details>` disclosure cleanup shipped as `webapp@8d9fe89`. See `journal/260512-storylines-cite-text.md` for the server-side change and the W10 retread evidence.
