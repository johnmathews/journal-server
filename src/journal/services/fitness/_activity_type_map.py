...
``strength`` / ``other``) is what ``fitness_activities.activity_type``
stores and what the locked-in correlation queries (`fitness-schema.md`
§8 Q2) join on. The verbatim source enum lives in
``fitness_activities.source_subtype`` so future queries that need
finer-grained filtering (e.g. trail-runs only, weekly rowing volume)
can do that without a schema change.
...
test #6 in W7 (``test_normalize.py``) parametrises the §3 table and
will fail on drift. Garmin's enum is less stable so its mapping is
maintained here in code; new typeKey strings encountered in
production fall back to ``other`` and surface as `source_subtype`
values that ops can review.
"""
...
def coarse_strava(sport_type: str) -> FitnessActivityType:
    ...
    Anything not in the §3 table (Rowing, Yoga, AlpineSki, RockClimbing,
    NordicSki, etc.) falls through to ``"other"``. The verbatim
    ``sport_type`` is preserved as ``source_subtype`` for downstream
    queries.
    """
