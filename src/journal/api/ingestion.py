...
Two sibling modules carry the rest of the override family (split out
when this file outgrew the ~800-line size rule):

- ``api/storylines_write.py`` — storyline create/regenerate/delete/anchors
- ``api/fitness_jobs.py``     — fitness sync + backfill jobs
...
job that does work", it goes here (or in the matching sibling write
module) even if its URL nests under another resource. New deviation
categories require updating both ``code-quality-principles.md`` and
``api/_shared.py``.
"""
